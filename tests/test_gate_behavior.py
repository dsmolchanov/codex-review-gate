...
def round_fixture(*prior, head_review: bool):
    ...
    fx = base_fixture()
    # ORDER IS LOAD-BEARING. The stub answers with the first substring key it
    # finds in insertion order, and this URL carries two filters that overlap:
    # the verdict read (review_rows_for_head) contains BOTH `commit_id == "` and
    # `submitted_at`, while the round-state read contains only `submitted_at`.
    # With `submitted_at` first, the verdict read is answered with the PRIOR
    # HEADS instead of this head's review — RIDS then holds 40-hex SHAs, the
    # marker scan builds `pull_request_review_id == aaaa…` (a bare jq
    # identifier), jq refuses to compile, and every degraded-round and
    # review-debt case fails for a reason that has nothing to do with the gate.
    fx["routes"]["pulls/7/reviews"] = {
...
def retraction_fixture(clean_ts, review_row, *, clean_count=0, prior=()):
    ...
    * `issues/7/comments` — `capture(` is the round-state read, CLEAN_TS_KEY is
    ...
    comments["*"] = ""
    fx["routes"]["issues/7/comments"] = comments
    return fx
...
def debt_title(path: str, body: str) -> str:
    """The title the gate will compute for this record — fingerprint included.
...
def test_a_long_path_cannot_push_the_title_past_the_api_limit(tmp_path):
    """A rejected POST lands in the fail-soft branch and the P1 vanishes.

    GitHub caps an issue title at 256 characters and `gh -f` truncates nothing,
    so an uncapped title turns "record the debt" into "warn and merge".
    """
    long_path = "src/" + "nested/" * 20 + "module.py"
    assert len(long_path) > 100
    fx = round_fixture(OLD1, OLD2, OLD3, head_review=True)
    fx["routes"]["pulls/7/comments"] = {
        "pull_request_review_id": debt_record(long_path, "**![P1 Badge](x) " + "y" * 200 + "**"),
        "*": "",
    }
    result = run_gate(tmp_path, fx)
    assert result.returncode == 0, f"{result.stdout}\n{result.stderr}"
    titles = [
        line.split("-f title=", 1)[1].split(" -f body=", 1)[0]
        for line in posted_bodies(tmp_path).splitlines()
        if "-f title=" in line
    ]
    assert titles, "nothing was filed"
    for title in titles:
        assert len(title) <= 256, f"title is {len(title)} chars: {title}"


def test_titles_that_collide_after_truncation_stay_distinct(tmp_path):
    """Two findings agreeing in their first 200 characters are still two.

    Under a display-text key the first files and the second is skipped, so a
    real P1 merges with no record — silently, which is the whole failure mode.
    """
    shared = "z" * 220
    fx = round_fixture(OLD1, OLD2, OLD3, head_review=True)
    fx["routes"]["pulls/7/comments"] = {
        "pull_request_review_id": (
            debt_record("api/x.py", f"**![P1 Badge](x) {shared} first**")
            + "\n"
            + debt_record("api/x.py", f"**![P1 Badge](x) {shared} second**", url="https://example/c/2")
        ),
        "*": "",
    }
    result = run_gate(tmp_path, fx)
    assert result.returncode == 0, f"{result.stdout}\n{result.stderr}"
    assert "filed 2, already open 0" in result.stdout, result.stdout


def test_badge_html_never_reaches_the_title(tmp_path):
    """Codex wraps its badge in <sub> tags, which markdown stripping leaves."""
    fx = round_fixture(OLD1, OLD2, OLD3, head_review=True)
    fx["routes"]["pulls/7/comments"] = {
        "pull_request_review_id": debt_record(
            "api/x.py",
            "**<sub><sub>![P1 Badge](https://img.shields.io/badge/P1-orange)</sub></sub>  Recheck the head**",
        ),
        "*": "",
    }
    run_gate(tmp_path, fx)
    calls = posted_bodies(tmp_path)
    assert "-f title=[review-debt] api/x.py: Recheck the head (" in calls, calls
    assert "<sub>" not in calls.split("-f body=")[0]
...
def test_a_bots_deferred_p1_merges_and_is_not_recorded(tmp_path):
    ...
    The bot replaces its pull request wholesale on the next run, so a record
    filed against it outlives the change it describes: measured 2026-09-11, 15
    of the 17 records naming a pull request that closed unmerged were on a
    stale dependabot branch. The gate itself still runs here — a dependency
    bump is exactly where review earns its cost — only the write is skipped.
    """
    fx = round_fixture(OLD1, OLD2, OLD3, head_review=True)
    fx["routes"]["pulls/7/comments"] = {
        "pull_request_review_id": debt_record("api/x.py", P1_BODY_NO_BLOCKER),
    ...
    fx["routes"]["pulls/7"] = {
        ...
        ".head.sha": HEAD,
        "*": json.dumps({"head": {"sha": HEAD}, "user": {"login": "dependabot[bot]"}}),
    }
    result = run_gate(tmp_path, fx)

    assert result.returncode == 0, f"{result.stdout}\n{result.stderr}"
    assert "Review debt is not recorded for dependabot[bot]" in result.stdout
    assert not [c for c in posted_bodies(tmp_path) if "[review-debt]" in c], (
        "a bot's pull request must leave no debt record"
    )


def test_a_human_authors_deferred_p1_is_still_recorded(tmp_path):
    """The exemption is by author, and nothing else about the round changes."""
    fx = round_fixture(OLD1, OLD2, OLD3, head_review=True)
    fx["routes"]["pulls/7/comments"] = {
        "pull_request_review_id": debt_record("api/x.py", P1_BODY_NO_BLOCKER),
        "*": "",
    ...
    fx["routes"]["pulls/7"] = {
        ".user.login": "dsmolchanov",
        ".head.sha": HEAD,
        "*": json.dumps({"head": {"sha": HEAD}, "user": {"login": "dsmolchanov"}}),
    }
    result = run_gate(tmp_path, fx)

    assert result.returncode == 0, f"{result.stdout}\n{result.stderr}"
    assert "is not recorded for" not in result.stdout
    assert "[review-debt] api/x.py: Unbounded request body (" in posted_bodies(tmp_path)


def test_an_unreadable_author_still_records_the_finding(tmp_path):
    """Fail-soft toward FILING: an unrecorded finding is the worse loss."""
    fx = round_fixture(OLD1, OLD2, OLD3, head_review=True)
    fx["routes"]["pulls/7/comments"] = {
        "pull_request_review_id": debt_record("api/x.py", P1_BODY_NO_BLOCKER),
        "*": "",
    }
    # The author read comes back empty; everything else this route serves is
    # unchanged, so only the author is unknown.
    fx["routes"]["pulls/7"] = {
        ".user.login": "",
        ".head.sha": HEAD,
...
def test_an_already_recorded_finding_is_not_refiled(tmp_path):
    ...
    fx["routes"]["issues?state=open"] = {"*": debt_title("api/x.py", P1_BODY_NO_BLOCKER)}
    result = run_gate(tmp_path, fx)
    assert result.returncode == 0, f"{result.stdout}\n{result.stderr}"
    calls = posted_bodies(tmp_path)
